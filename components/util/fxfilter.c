// Filter brick — see fxfilter.h. Multimode insert filter over the shared svf
// (trapezoidal since 2026-09-16: stable across this brick's full 12 kHz range).
#include <math.h>
#include "fxfilter.h"

#define FLT_RATE 44100.0f

// stable by construction; this only catches a NaN fed in from upstream
static inline void guard(fxfilter_t *fl, float cf)
{
    if (!(fabsf(fl->l.lp) < 1e6f) || !(fabsf(fl->l.bp) < 1e6f) ||
        !(fabsf(fl->r.lp) < 1e6f) || !(fabsf(fl->r.bp) < 1e6f) || !(fl->cf_slew >= 0.0f && fl->cf_slew < 20.0f)) {
        svf_reset(&fl->l); svf_reset(&fl->r); fl->cf_slew = cf;
    }
}

void fxfilter_block_f(fxfilter_t *fl, float *buf, int frames)
{
    int   mode = fl->mode; if (mode < 0 || mode >= FILT_NMODE) mode = FILT_LP;
    float cut  = fl->cutoff; if (cut < 0) cut = 0; else if (cut > 1) cut = 1;
    float reso = fl->reso;   if (reso < 0) reso = 0; else if (reso > 1) reso = 1;

    float fc = 30.0f * powf(400.0f, cut);          // 30 Hz .. ~12 kHz (log)
    float cf = svf_coef(fc, FLT_RATE, 2.0f);       // no ceiling: stable to the top
    // q is DAMPING (higher = cleaner). reso 0 -> 2.0 (clean), 1 -> 0.3 (resonant)
    float q  = 2.0f - reso * 1.7f;
    guard(fl, cf);

    for (int f = 0; f < frames; f++) {
        fl->cf_slew += 0.05f * (cf - fl->cf_slew);  // de-zipper the sweep
        float xl = buf[f * 2], xr = buf[f * 2 + 1];
        float lpl, bpl, hpl, lpr, bpr, hpr;
        svf_step(&fl->l, xl, fl->cf_slew, q, &lpl, &bpl, &hpl);
        svf_step(&fl->r, xr, fl->cf_slew, q, &lpr, &bpr, &hpr);
        buf[f * 2]     = mode == FILT_LP ? lpl : mode == FILT_HP ? hpl : bpl;
        buf[f * 2 + 1] = mode == FILT_LP ? lpr : mode == FILT_HP ? hpr : bpr;
    }
}

void fxfilter_band_block_f(fxfilter_t *fl, float *buf, int frames)
{
    float base  = fl->cutoff; if (base < 0) base = 0; else if (base > 1) base = 1;
    float width = fl->reso;   if (width < 0) width = 0; else if (width > 1) width = 1;
    float fc = 30.0f * powf(400.0f, base);            // center: 30 Hz .. ~12 kHz
    float cf = svf_coef(fc, FLT_RATE, 2.0f);
    float q  = 0.3f + width * 3.7f;                   // narrow/resonant .. wide/gentle
    guard(fl, cf);
    for (int f = 0; f < frames; f++) {
        fl->cf_slew += 0.05f * (cf - fl->cf_slew);
        float xl = buf[f * 2], xr = buf[f * 2 + 1];
        float lpl, bpl, hpl, lpr, bpr, hpr;
        svf_step(&fl->l, xl, fl->cf_slew, q, &lpl, &bpl, &hpl);
        svf_step(&fl->r, xr, fl->cf_slew, q, &lpr, &bpr, &hpr);
        buf[f * 2]     = bpl;                          // bandpass tap
        buf[f * 2 + 1] = bpr;
    }
}
