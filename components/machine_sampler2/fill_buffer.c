...
void s2_fill_audio_buffer(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file, SemaphoreHandle_t* mutex){
    ...
    // sd_lock is nested INSIDE file_mutex (consistent ordering, no deadlock); it
    // shares the SD bus with REST/recording so raw f_read can't race their I/O
    if(mutex != NULL)
    {
...
void s2_fill_buffer_one_shot(void* _playback_engine, void* _buffer, void* _file){
    UINT n = 0;
    ...
    else // Playback is backwards
    {
        ...
        if(file->fpos <= playback_engine->sample_start){ // if file reached the beginning just load zeros to buffer
            ...
            f_read(&(file->fil), buffer->buf, samps_to_read, &n);
            s2_reverseBuffer32((n / 4), buffer->buf); // reverse buffer in place
            file->fpos -= n;
        }
    }
...
void s2_jump_to_start(voice_play_mode_t* playback_engine, audio_b_t* first_buffer,audio_b_t* second_buffer, audio_f_t* file, SemaphoreHandle_t* mutex){
    bool play_dir = playback_engine->is_playback_direction_forward;
...
void s2_refill_first_buf(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file, SemaphoreHandle_t* mutex){
    bool play_dir = playback_engine->is_playback_direction_forward;
    xSemaphoreTake(*mutex, portMAX_DELAY);
    sd_lock_take();
...
void s2_init_play_mode(voice_play_mode_t* pm){
    pm->is_pipo_playback_forward = 0;
    pm->is_playback_direction_forward = 1;
    pm->loop_end = 0;
    pm->loop_start = 0;
    pm->sample_start = 0;
    pm->play_mode = &s2_fill_buffer_one_shot;
    pm->phase = 0;
...
void s2_fill_buffer_fwd_loop(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file){
...
void s2_fill_buffer_bwd_loop(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file){
    ...
    if(file_pos - SD_BUF_SZ >= loop_start){
        ...
        f_read(&(file->fil), buffer->buf, SD_BUF_SZ, &n);
        s2_reverseBuffer32((n / 4), buffer->buf);
        file->fpos -= n;
        if(file->fpos <= 0){
            file->fpos = loop_end;
        }
...
void s2_fill_buffer_loop(void* _playback_engine, void* _buffer, void* _file){
    voice_play_mode_t* playback_engine = (voice_play_mode_t*) _playback_engine;
    audio_b_t* buffer = (audio_b_t*) _buffer;
    audio_f_t* file = (audio_f_t*) _file;
    bool play_dir = playback_engine->is_playback_direction_forward;
    ...
    if(play_dir){
        s2_fill_buffer_fwd_loop(playback_engine, buffer, file);
    }
    else{
        s2_fill_buffer_bwd_loop(playback_engine, buffer, file);
    }
}
...
void s2_fill_buffer_fwd_pipo(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file){
    int loop_start = playback_engine->loop_start;
    int loop_end = playback_engine->loop_end;
    UINT n = 0;
    ...
    }else
    {
        ...
        n += tempn;
        f_lseek(&(file->fil), seek_pos);
        s2_reverseBuffer32((samps_to_reverse / 4), (buffer->buf + samps_remain));
        file->fpos = seek_pos;
        playback_engine->is_pipo_playback_forward = false;
    }
...
void s2_fill_buffer_bwd_pipo(voice_play_mode_t* playback_engine, audio_b_t* buffer, audio_f_t* file){
    int loop_start = playback_engine->loop_start;
    int loop_end = playback_engine->loop_end;
    UINT n = 0;
    int file_pos = file->fpos;
...
void s2_fill_buffer_pipo(void* _playback_engine, void* _buffer, void* _file){
    voice_play_mode_t* playback_engine = (voice_play_mode_t*) _playback_engine;
    audio_b_t* buffer = (audio_b_t*) _buffer;
    audio_f_t* file = (audio_f_t*) _file;
    bool play_dir = playback_engine->is_playback_direction_forward;
    ...
    if((play_dir && play_dir_pipo) || (!play_dir && play_dir_pipo)){
        s2_fill_buffer_fwd_pipo(playback_engine, buffer, file);
    } else if((!play_dir && !play_dir_pipo) || (play_dir && !play_dir_pipo)){
        s2_fill_buffer_bwd_pipo(playback_engine, buffer, file);
    }
}
