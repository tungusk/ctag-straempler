...
void s2_updateAttackPoint(vec2d_t* curve, vec2d_t* prevCurve, adsr_data_t* data, vec2d_t* startPoint, const int* width, const int* height){
...
void s2_updateDecayPoint(vec2d_t* curve, vec2d_t* prevCurve, adsr_data_t* data, vec2d_t* startPoint, const int* width, const int* height){
...
void s2_updateSustainPoint(vec2d_t* curve, vec2d_t* prevCurve, adsr_data_t* data, const int* width, const int* height){
...
void s2_updateReleasePoint(vec2d_t* curve, vec2d_t* prevCurve, adsr_data_t* data, const int* width, const int* height){
...
void s2_updatePrevCurve(vec2d_t* curve, vec2d_t* prevCurve){
    for(int i = 0; i < 4; i++)
    {
...
void s2_resetPrevCurve(vec2d_t* prevCurve){
    for(int i = 0; i < 4; i++)
    {
        prevCurve[i].x = 2;
        prevCurve[i].y = 2;
...
void s2_printIndicatorPoints(vec2d_t* curve, vec2d_t* startPoint, int radius, color_t color){
