...
void s2_init_ui_params(ui_param_holder_t* params){
...
void s2_parse_play_direction(xQueueHandle* queue, TaskHandle_t* task_handle, voice_t* voice, int vid){
    bool dir;
...
void s2_parse_voice_param_data(xQueueHandle* queue, voice_t* voice, ui_param_holder_t* ui_params, param_data_t* params){
    if(xQueueReceive(*queue ,params, 0)){
            ...
            if(!ui_params->is_pitch_cv_active){
                ...
                voice->playback_engine.pitch_increment = s2_pitch_lut_24[params->pitch];
            ...
            if(ui_params->is_shaper_active)
                ui_params->dist_amp = s2_dist_q_lut[params->dist_drive];
            ...
            ui_params->resonance = s2_dist_q_lut[params->filter.q];   
     }
}

void s2_parse_play_state_data(xQueueHandle* queue, play_state_data_t* play_state_data, ui_param_holder_t* ui_params, TaskHandle_t* task_handle, voice_t* voice, audio_f_t* file, void (**play_modes)(void*,void*,void*), int vid){
