// sampler machine — menu pages, preset schema, and main-screen live area.
// Physically split out of components/menu in M0c-2.
#include <time.h>
#include <sys/time.h>
#include <unistd.h>
#include <string.h>
...
void s2_initParams(void);   // defined below; the original leaned on a stale core menu.h decl
...
static int sampler_main_event(int event, void *event_data){
    switch(event){
        case EV_ENTERED_MENU:
            s2_menuTFTInvalidatePlayArea();   // screen was repainted underneath the live displays
        ...
        case EV_UPDATE_V0_POS:
            s2_menuTFTUpdatePlayState(0, (int) event_data);
            break;
        case EV_UPDATE_V1_POS:
            s2_menuTFTUpdatePlayState(1, (int) event_data);
            break;
        default:
            break;
    }
...
static int browse_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            no_wifi = 0;
            menuTFTPrintMenuH(s2_browse_menus, &s2_n_browse_menus);
            menuTFTSelectMenuItemH(&menu_state_current, 0, s2_browse_menus, &s2_n_browse_menus);
            break;
        case EV_FWD:
            if(no_wifi) break;
            menu_state_current++;
...
static int play_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            menuTFTPrintMenu(s2_play_menus, &s2_n_play_menus);
            menuTFTSelectMenuItem(&menu_state_current, 0, s2_play_menus, &s2_n_play_menus);
            break;
        case EV_FWD:
            menu_state_current++;
            if(menu_state_current >= states) menu_state_current = 0;
...
static int voice_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            s2_menuTFTPrintVoiceMenu();
            s2_menuTFTSelectVoiceMenu(menu_pos,selected);
            s2_menuTFTPrintVoiceValues(&data[vid], PRINT_ALL);
            break;
        case EV_FWD:
           if(!selected){
                menu_pos++;
                ...
                if(vid)
                    s2_incParamValue(&data[vid], menu_items[menu_pos], vid, &pbs_state_v1, matrix, playbackspeed_state_queue_v1);
                else
                    s2_incParamValue(&data[vid], menu_items[menu_pos], vid, &pbs_state_v0, matrix, playbackspeed_state_queue_v0);
                s2_menuTFTPrintVoiceValues(&data[vid],menu_items[menu_pos]);
        ...
        case EV_BWD:
            if(!selected){
                ...
                if(vid)
                    s2_decParamValue(&data[vid], menu_items[menu_pos],vid, &pbs_state_v1, matrix, playbackspeed_state_queue_v1);
                else
                    s2_decParamValue(&data[vid], menu_items[menu_pos],vid, &pbs_state_v0, matrix, playbackspeed_state_queue_v0);
                s2_menuTFTPrintVoiceValues(&data[vid],menu_items[menu_pos]);
        ...
        case EV_SHORT_PRESS:
            if(!selected){
                ...
                if(data[vid].trig_mode_latch)
                    s2_enableTrigModeLatch(vid);
                else
                    s2_disableTrigModeLatch(vid);
                break;
            }
            break;
        case EV_LONG_PRESS:
...
static void filter_change(int sid, int vid, int delta, void *ctx) {
    ...
    if (delta > 0) s2_incFilterValue(f, sid); else s2_decFilterValue(f, sid);
    s2_menuTFTPrintFilterValues(f, sid);
...
static int filter_def_handler(int it_id, int event, void* event_data) {
    ...
    static menu_nav_t nav = {
        .labels = s2_filter_menus, .sids = sids, .n = sizeof(sids)/sizeof(sids[0]),
        .parent = M_VOICE, .change = filter_change,
    };
    if (event == EV_ENTERED_MENU) {
        vid = (int)_state_voice;
        nav.ctx = &data[vid].filter;
        menuTFTPrintMenu(s2_filter_menus, &s2_n_filter_menus);
        menuTFTSelectMenuItem(&pos, sel, s2_filter_menus, &s2_n_filter_menus);
        s2_menuTFTPrintFilterValues(&data[vid].filter, PRINT_ALL);
    }
...
static void adsr_change(int sid, int vid, int delta, void *ctx) {
    ...
    else if (delta > 0) s2_incADSRValue(&data[vid].adsr, envelopeIndex[vid], sid);
    else                s2_decADSRValue(&data[vid].adsr, envelopeIndex[vid], sid);
    s2_menuTFTPrintADSRValues(&data[vid].adsr, sid);
    s2_menuTFTPrintADSRCurve(&data[vid].adsr);
    xQueueSend(vid ? v1_queue : v0_queue, &data[vid], portMAX_DELAY);
}
...
static int adsr_def_handler(int it_id, int event, void* event_data) {
    ...
    static int pos = 0, sel = 0, vid = 0;
    static menu_nav_t nav = {
        .labels = s2_adsr_menus, .sids = sids, .n = sizeof(sids)/sizeof(sids[0]),
        .parent = M_VOICE, .change = adsr_change,
    };
    if (event == EV_ENTERED_MENU) {
        vid = (int)_state_voice;
        s2_menuTFTPrintADSRMenu();
        menuTFTSelectMenuItem(&pos, sel, s2_adsr_menus, &s2_n_adsr_menus);
        s2_menuTFTPrintADSRValues(&data[vid].adsr, PRINT_ALL);
        s2_menuTFTInitAdsrCurve();
        s2_menuTFTPrintADSRCurve(&data[vid].adsr);
    }
    return menu_nav_step(&nav, &pos, &sel, vid, event);
}
...
static void playmode_change(int sid, int vid, int delta, void *ctx) {
    ...
    if (delta > 0) s2_incPlaymodeValue(&data[vid].play_state, sid, mq);
    else           s2_decPlaymodeValue(&data[vid].play_state, sid, mq);
    s2_menuTFTPrintPlaymodeValues(&data[vid].play_state, sid);
    s2_menuTFTPrintPlaymodeIndicators(&data[vid].play_state, sid);
...
static int playmode_def_handler(int it_id, int event, void* event_data) {
    ...
    static int pos = 0, sel = 0, vid = 0;
    static menu_nav_t nav = {
        .labels = s2_playmode_menus, .sids = sids, .n = sizeof(sids)/sizeof(sids[0]),
        .parent = M_VOICE, .change = playmode_change,
    };
    if (event == EV_ENTERED_MENU) {
        vid = (int)_state_voice;
        s2_menuTFTPrintPlaymodeMenu();
        menuTFTSelectMenuItem(&pos, sel, s2_playmode_menus, &s2_n_playmode_menus);
        s2_menuTFTPrintPlaymodeValues(&data[vid].play_state, PRINT_ALL);
        s2_menuTFTInitPlaymodeIndicators();
        s2_menuTFTPrintPlaymodeIndicators(&data[vid].play_state, sids[pos]);
    }
    return menu_nav_step(&nav, &pos, &sel, vid, event);
}
...
static int matrix_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            s2_menuTFTPrintMatrixMenu();
            s2_menuTFTSelectMatrixItem(matrix_pos_v, 0,matrix_pos_h);
            s2_menuTFTPrintMatrixAmount(matrix, PRINT_ALL);
            s2_menuTFTPrintMatrixDestination(matrix, PRINT_ALL);
        ...
        case EV_FWD: ;
            if(selected_h){
                ...
                if(matrix_pos_h == 1){
                    s2_incItemAmount(matrix, matrix_pos_v);
                    //Print matrix amount
                ...
                if(matrix_pos_h == 2){
                    dst_changed = 1;
                    s2_incDestination(matrix, matrix_pos_v);
                    s2_menuTFTPrintMatrixDestination(matrix, matrix_pos_v);
                    s2_menuTFTPrintMatrixAmount(matrix, matrix_pos_v);        
                }
        ...
        case EV_BWD: 
            if(selected_h){
                ...
                if(matrix_pos_h == 1){
                    s2_decItemAmount(matrix, matrix_pos_v);
                    s2_menuTFTPrintMatrixAmount(matrix, matrix_pos_v);
                } 
                if(matrix_pos_h == 2){
                    dst_changed = 1;
                    s2_decDestination(matrix, matrix_pos_v);
                    s2_menuTFTPrintMatrixDestination(matrix, matrix_pos_v);
                    s2_menuTFTPrintMatrixAmount(matrix, matrix_pos_v);
                } 
        ...
        case EV_SHORT_PRESS:
            if(!selected_v){
                selected_v = 1;
                s2_menuTFTPrintMatrixRowIndicator(selected_v, matrix_pos_v);
            }else if(selected_v){
                if(matrix_pos_h > 0){
                ...
                s2_menuTFTSelectMatrixItem(matrix_pos_v, selected_h, matrix_pos_h);
            }
            break;
        case EV_LONG_PRESS:
            matrix_pos_v = 0;
...
static int effects_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        ...
        case EV_ENTERED_MENU:
            menuTFTPrintMenu(s2_effect_menus, &s2_n_effect_menus);
            menuTFTSelectMenuItem(&menu_state_current, 0, s2_effect_menus, &s2_n_effect_menus);
            break;
        case EV_FWD:
            menu_state_current++;
            if(menu_state_current >= states) menu_state_current = 0;
...
static void extin_change(int sid, int vid, int delta, void *ctx) {
    ...
    if (delta > 0) s2_incExtInValue(e, sid); else s2_decExtInValue(e, sid);
    s2_menuTFTPrintExtInValues(e, sid);
...
static int extin_def_handler(int it_id, int event, void* event_data) {
    ...
    static menu_nav_t nav = {
        .labels = s2_externel_in_menus, .sids = sids, .n = sizeof(sids)/sizeof(sids[0]),
        .parent = M_PLAY, .change = extin_change,
        .ctx = &effectData.extInData,
    };
    if (event == EV_ENTERED_MENU) {
        menuTFTPrintMenu(s2_externel_in_menus, &s2_n_externel_in_menus);
        menuTFTSelectMenuItem(&pos, sel, s2_externel_in_menus, &s2_n_externel_in_menus);
        s2_menuTFTPrintExtInValues(&effectData.extInData, PRINT_ALL);
    }
...
static int recording_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            s2_menuTFTPrintRecordingState(recording_get_trig_func(0), recording_get_trig_func(1));
            break;
        case EV_FWD:
            if(!selected){
...
static void delay_change(int sid, int vid, int delta, void *ctx) {
    ...
    if (delta > 0) s2_incDelayValue(d, sid); else s2_decDelayValue(d, sid);
    s2_menuTFTPrintDelayValues(d, sid);
    xQueueSend(eff_queue, &effectData, portMAX_DELAY);
}
...
static int delay_def_handler(int it_id, int event, void* event_data) {
    ...
    static int pos = 0, sel = 0;
    static menu_nav_t nav = {
        .labels = s2_delay_menus, .sids = sids, .n = sizeof(sids)/sizeof(sids[0]),
        .parent = M_EFFECTS, .change = delay_change,
        .ctx = &effectData.delay,
    };
    if (event == EV_ENTERED_MENU) {
        menuTFTPrintMenu(s2_delay_menus, &s2_n_delay_menus);
        menuTFTSelectMenuItem(&pos, sel, s2_delay_menus, &s2_n_delay_menus);
        s2_menuTFTPrintDelayValues(&effectData.delay, PRINT_ALL);
    }
    return menu_nav_step(&nav, &pos, &sel, 0, event);
}
...
static int bank_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            s2_menuTFTPrintSlotMenu(cfgData, activeSlot);
...
static int userfilebrowser_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        ...
        case EV_SHORT_PRESS:
            ...
            break;
        case EV_TIMER_REPEATING_FAST:
            s2_menuTFTAnimateFileBrowser(info);
            break;
        default:
            break;
    }
...
static int decoding_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            id = (char*) _state_data;
            s2_menuTFTPrintDecoding();
            s2_menuTFTUpdateProgress("Decoding", 0);
            decodeMP3File(id);
            break;
        case EV_DECODING_PROGRESS:
            s2_menuTFTUpdateProgress("Decoding", (int) event_data);
            break;
...
static int browse_user_def_handler(int it_id, int event, void* event_data){
    switch(event){
        case EV_ENTERED_MENU:
            s2_menuTFTPrintUserFileMenu();
            break;
        case EV_DECODING_PROGRESS:
            s2_menuTFTUpdateProgress("User upload", (int)event_data);
            break;
        case EV_LONG_PRESS:
            menuTFTFlushMenuDataRect();
            return M_MAIN;
...
static int browse_tag_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            isBusy = true;
            s2_menuTFTPrintLoadingTagMenu();
            freesoundGetTags("");
            selected_tags_list = list_create();
            break;
        case EV_PROGRESS_UPDATE:
            s2_menuTFTUpdateProgress("Download", (int)event_data);
            break;
        case EV_FREESND_TAGLIST:
            tag_list = (list_t*)event_data;
            s2_menuTFTPrintBrowseTagList(tag_list);
            isBusy = false;
            break;
        case EV_FWD:
...
static int preset_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            ...
            menuTFTPrintMenuH(s2_preset_menus, &s2_n_preset_menus);
            menuTFTSelectMenuItemH(&menu_state_current, 0, s2_preset_menus, &s2_n_preset_menus);
            break;
        case EV_FWD:
            menu_state_current++;
            if(menu_state_current >= states) menu_state_current = 0;
...
static int typeselect_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            //ESP_LOGI("PRESET", "Entered preset top level menu");
            menuTFTPrintMenuH(s2_slot_menus, &s2_n_slot_menus);
            menuTFTSelectMenuItemH(&menu_state_current, 0, s2_slot_menus, &s2_n_slot_menus);
            break;
        case EV_FWD:
            menu_state_current++;
            if(menu_state_current >= states) menu_state_current = 0;
...
static int presets_menu_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            selected = 0;
            current_fb_pos = 0;
            menuTFTPrintMenuHSpaced(s2_preset_choices, &s2_n_preset_choices);
            menuTFTSelectMenuItemHSpaced(&menu_state_current, selected, s2_preset_choices, &s2_n_preset_choices);
            s2_menuTFTPrintCurrentPresetSettings("Current preset: ", current_preset_name);
            break;
        case EV_TIMER_COMPLETE:
            if(root == NULL){
                root = s2_getBankRoot(current_bank); 
                if(root != NULL)bank = cJSON_GetObjectItemCaseSensitive(root, "bank");
                else menuTFTPrintInputError("Error reading bank file");
                s2_menuTFTPrintPresetList(bank, PRINT_FAST);
            }
            break;
        case EV_FWD:
            if(selected){
                ...
                s2_menuTFTSelectPreset(&list_pos, bank, selected);
            }else{
                //In topmenu selection
                menu_state_current++;
                if(menu_state_current >= states) menu_state_current = 0;
                menuTFTSelectMenuItemHSpaced(&menu_state_current, 0, s2_preset_choices, &s2_n_preset_choices);
            }
            break;
        case EV_BWD:
            if(selected){
                    ...
                    if(!selected){
                        ...
                        switch (menu_states[menu_state_current])
                        {
                            case M_PRESET_LOAD:
                                ...
                                s2_loadParams(cJSON_GetArrayItem(bank, list_pos));
                                //Display feedback
                                fb_state = MS_SUCCESS;
                                setTimerTopmenuFeedback(250, ui_ev_queue);
                                s2_menuTFTPrintCurrentPresetSettings("Current preset: ", current_preset_name);  
                            ...
                            case M_PRESET_DELETE:
                                ...
                                if(strcasecmp(current_preset_name, s2_getPresetNameAtIndex(bank, &list_pos)) != 0){
                                    //Display feedback
                                    fb_state = MS_SUCCESS;
                                    //Preset to delete is not current preset
                                    s2_deletePreset(bank, &list_pos);
                                    //Save to bank
                                    s2_saveBank(root, current_bank, NO_ACTION);
                                    //Clearing list item
                                    if(list_pos != cJSON_GetArraySize(bank)){
                                        //deleted item not last in list, refresh
                                        s2_menuTFTPrintPresetList(bank, PRINT_CLEAR);
                            ...
                            case M_PRESET_STORE: ;
                                ...
                                cJSON* preset = s2_buildPreset(s2_getPresetNameAtIndex(bank, &list_pos));
        ...
        case EV_TIMER_MENU_FB:
            ...
            if(event_data != NULL)s2_menuTFTFeedbackMenuItemHSpaced((int*) event_data, &current_fb_pos, s2_preset_choices, &s2_n_preset_choices, fb_state);
...
static int banks_menu_def_handler(int it_id, int event, void* event_data){
    ...
    switch(event){
        case EV_ENTERED_MENU:
            selected = 0;
            current_fb_pos = 0;
            menuTFTPrintMenuHSpaced(s2_bank_choices, &s2_n_bank_choices);
            menuTFTSelectMenuItemHSpaced(&menu_state_current, 0, s2_bank_choices, &s2_n_bank_choices);
            s2_menuTFTPrintCurrentPresetSettings("Current bank: ", current_bank);
            break;
        case EV_TIMER_COMPLETE:
            if(file_list == NULL){
                file_list = list_create();
                getFilesInDir(file_list, "/sdcard/banks", ".JSN");
                s2_menuTFTPrintBankList(file_list, PRINT_FAST);
            }
            break;
        case EV_FWD:
            if(selected){
                ...
                s2_menuTFTSelectPresetBank(&list_pos, file_list,selected);
                break;
            }
            //In topmenu selection
            menu_state_current++;
        ...
        case EV_SHORT_PRESS:
            if(menu_states[menu_state_current] == M_PRESET_BANK_NEW){
                ...
                selected = 1;
                if(!submenu_active) submenu_active = 1;
                menuTFTSelectMenuItemHSpaced(&menu_state_current, selected, s2_bank_choices, &s2_n_bank_choices);
                    ...
                    case M_PRESET_BANK_DELETE: ;
                        ...
                        if(strcmp(bn, current_bank) != 0){
                            ...
                            if(list_pos != file_list->count){
                                //deleted item not last in list, refresh
                                s2_menuTFTPrintBankList(file_list, PRINT_CLEAR);
                            }else menuTFTClearListItem(&list_pos); //clear last item
        ...
        case EV_TIMER_MENU_FB:
            ...
            if(event_data != NULL)s2_menuTFTFeedbackMenuItemHSpaced((int*) event_data, &current_fb_pos, s2_bank_choices, &s2_n_bank_choices, fb_state);
            else ESP_LOGE("UI", "Error displaying feedback");
        default:
            break;
    }
            ...
            switch(c_list[c]){
                ...
                case '=':
                    ...
                    if(bank != NULL && cJSON_GetArraySize(bank) < MAX_PRESET_COUNT){
                        if(s2_validatePresetNameInput(buf, bank)){
                            //Input valid, build preset, then save to bank
                            preset = s2_buildPreset(buf);
                            s2_addPreset(bank, preset);
                            s2_saveBank(root, current_bank, DELETE_JSON);
                            root = NULL;
                            bank = NULL;
                            return M_PRESET_P;
                        }else{
...
void s2_initParams(){
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        data[i].adsr.attack = s2_msLut[2];
        data[i].adsr.decay = s2_msLut[4];
        data[i].adsr.sustain = 100;         //Q1.7; 0 = 0; 128 = 1.0; INCR: 0.0078125
        data[i].adsr.release = s2_msLut[4];
...
cJSON* s2_buildPreset(const char* name){
    char buf[20];
    cJSON *val;
...
static void initParamsFromPreset(char* presetName, char* bankName){
    ...
    cJSON *ar = s2_getBankRoot("autosave");
    if (ar) {
        cJSON *apreset = s2_getPresetByName("state", ar);
        if (apreset) {
            s2_loadParams(apreset);
    ...
    if(bank != NULL){
        ...
        //s2_getPresetByName returns detached object, make sure to free, else memory leak
        preset = s2_getPresetByName(presetName, root);         
        if(preset != NULL){
            s2_loadParams(preset);
            cJSON_Delete(preset);
...
// physically moves to s2_machine_sampler in M0c-2.
// ─────────────────────────────────────────────────────────────────────────────
...
{
    return s2_buildPreset("state");
}

void s2_sampler_preset_load(const cJSON *node)
{
    if (node) {
        s2_loadParams((cJSON *)node);
