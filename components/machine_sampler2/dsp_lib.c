...
void s2_init_filter(Filter_t* filter, filtertype_t f_type){
    ...
    s2_reset_filter(filter);

    s2_calculate_coeffs(filter);
}
...
void s2_calculate_coeffs(Filter_t *filter){
    ...
    float lambda = s2_fasttan(M_PI * cutoff / sample_rate);
...
void s2_filter_sound(Filter_t *filter, float* out_left, float* out_right){
...
float s2_distort_sound(WaveShaper_t* shaper, float value){
    if(shaper->is_active)
        return s2_fasttanh(value * shaper->amp_fator_positive);
    else
        return value;
}
...
void s2_set_cutoff_values(voice_t* voice, uint16_t* ctrl_data, int vid)
{
    ...
    if(vid)
    {
        ...
        if(offset_pos < 0)
        {
            ...
            offset = -1 * s2_negative_freq_lut[(-1 * offset_pos)];
        }
        else{
            offset = s2_poti_vals[(offset_pos)];
        }
        cutoff_hp = s2_poti_vals[(ctrl_data[inner_right] >> 3)] + offset;
        cutoff_lp = s2_poti_vals[511 - (ctrl_data[inner_right] >> 3)] + offset;
    }
    else
    {
        ...
        cutoff_hp = s2_poti_vals[(ctrl_data[outer_left] >> 3)] + offset;
        cutoff_lp = s2_poti_vals[511 - (ctrl_data[outer_left] >> 3)] + offset;
    }
    ...
    s2_set_cutoff(voice->highpass_filter, cutoff_hp);
    s2_set_cutoff(voice->lowpass_filter, cutoff_lp);
...
void s2_set_filter_values_ui(voice_t* voice, uint16_t base, uint16_t width, float q, int vid){
    ...
    cutoff_hp = s2_poti_vals[base] - s2_poti_vals[width] / 2;
    cutoff_lp = s2_poti_vals[base] + s2_poti_vals[width] / 2;
    ...
    s2_set_filter_params(voice->highpass_filter, cutoff_hp, q);
    s2_set_filter_params(voice->lowpass_filter, cutoff_lp, q);
}

inline void s2_apply_voice_fx(float* buf, int len, voice_t* voice, uint16_t* ctrl_data, int vid, float vol){
    float sample_left, sample_right;
    s2_set_cutoff_values(voice, ctrl_data, vid);
    ...
    for(int i = 0; i < len/2; i++)
    {
        ...
        // sample_left = s2_fasttanh(dist_amp * sample_left);
        // sample_right = s2_fasttanh(dist_amp * sample_right);
        

        if(voice->lowpass_filter->is_active)             
            s2_filter_sound(voice->lowpass_filter, &sample_left, &sample_right);         
        if(voice->highpass_filter->is_active)             
            s2_filter_sound(voice->highpass_filter, &sample_left, &sample_right);
        // float amp = (ctrl_data[outer_left] >> 3) / (float) 511 * 2;

        
        
        // s2_stereo_balance(ctrl_data, &sample_left, &sample_right);
...
void s2_init_shaper(WaveShaper_t* shaper){
    shaper->is_active = 0;
    s2_set_distortion_amp(shaper, 0.2);
}
...
void s2_process_right_channel(PanPos_t* pan, float* right_channel)
{
    ...
    inline void s2_buffer_copy(float* dst, int dst_start, float* src, int src_start, int len){
    for(int i = 0; i < len; i++){
        dst[dst_start + i] = src[src_start + i];
    }
}

inline void s2_buffer_copy_stereo(float* dst, int32_t dst_start,float* src, int32_t src_start, int32_t len, int32_t channel){
    for(int i = 0; i < len / 2; i++){
