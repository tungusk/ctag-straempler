...
void s2_fifo_init(fifo_t* f, int32_t* buf, size_t size);

/**
 * @brief Function to read a Sample from a given Fifo and delete it in next call of s2_fifo_write()
 * 
...
int s2_fifo_pop_element(fifo_t* f, int32_t* sample);
...
int s2_fifo_write(fifo_t* f, const int32_t* buf, int n_int_32_elements);
...
int s2_fifo_read_element(fifo_t* f, int32_t* sample);
...
void s2_fifo_drop_samples(fifo_t* f, int32_t num_samples);