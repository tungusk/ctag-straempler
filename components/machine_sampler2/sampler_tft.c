// sampler machine — display routines, split out of components/menu/menutft.c
// in M0c-2b. Uses core primitives from menutft_utils and the tft library.
#include <string.h>
#include <time.h>
#include <sys/time.h>
#include <ctype.h>
...
// caches for the main-screen live displays; reset via s2_menuTFTInvalidatePlayArea()
...
void s2_menuTFTPrintVoiceMenu(){
    ...
    _cur_row = 0;
    _fg = TFT_LIGHTGREY;
    for(i=0; i<s2_n_voice_menus; i++){
        if(i == 9){
            TFT_drawFastHLine(2, ((TFT_getfontheight() + 3) *_cur_row) + (TFT_getfontheight() / 2) + 1, _width - 2, TFT_WHITE);
            _cur_row++;   
        }
        TFT_print((char *)s2_voice_menus[i], x, 3 + (TFT_getfontheight() + 3) *_cur_row);
        _cur_row++;
    }
}
...
void s2_menuTFTPrintADSRMenu(){
    ...
    _cur_row = 0;
    _fg = TFT_LIGHTGREY;
    for(i=0; i<s2_n_adsr_menus; i++){
        TFT_print((char *)s2_adsr_menus[i], x, 3 + (TFT_getfontheight() + 3) *_cur_row);
        _cur_row++;
    }
    _cur_row += 3;
...
void s2_menuTFTPrintPlaymodeMenu(){
    ...
    _cur_row = 0;
    _fg = TFT_LIGHTGREY;
    for(i=0; i<s2_n_playmode_menus; i++){
        TFT_print((char *)s2_playmode_menus[i], x, 3 + (TFT_getfontheight() + 3) *_cur_row);
        _cur_row++;
    }
...
void s2_menuTFTPrintMatrixMenu(){
    ...
    //Draw top row
    for(int i = 0; i < s2_n_cv_matrix_items; i++){
        w = TFT_getStringWidth((char*)s2_cv_matrix_items[i]);
        TFT_setclipwin(x,TFT_getfontheight()+9, x + x_inc, _height);
        //TFT_fillWindow(TFT_RED);
        TFT_print((char *)s2_cv_matrix_items[i], CENTER, 4);
        x+= x_inc;
        TFT_resetclipwin();
    }
...
void s2_menuTFTSelectVoiceMenu(int active, int select){
    int h = 0, y = 0;
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    _fg = TFT_WHITE;
    _bg = TFT_BLACK;
    TFT_X = 0;
    h = TFT_getfontheight();
    for(int i=0;i<s2_n_voice_menus;i++){
        _bg = TFT_BLACK;
        if(i==active){
            if(select == 1){
                _bg = TFT_RED;
        ...
        TFT_drawRect(2, y, TFT_getStringWidth((char*)s2_voice_menus[i]) + 4, h + 3 , _bg);
        y += h + 3;
        if(i == 8) y += h + 3;
    }
}
...
void s2_menuTFTSelectPreset(int* activeSlot, cJSON* rootArray, int selected){
    ...
    //ESP_LOGI("s2_menuTFTSelectPreset", "cJSON Array Size: %d", cJSON_GetArraySize(rootArray));
    int i = 0;
    if(rootArray != NULL){
        cJSON_ArrayForEach(element, rootArray){
            _bg = TFT_BLACK;
            if(i==*activeSlot){
                //ESP_LOGI("s2_menuTFTSelectPresetBank", "i == activeSlot: %d | Value: %s", i, (char*)list_get_item(list, i)->value);
...
void s2_menuTFTPrintVoiceValues(param_data_t* data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    ...
    {
        case SID_TRIG_TYPE:
            sprintf(tmp, s2_trig_items[data->trig_mode_latch]);
            menuTFTFlush(0, &_bg);
        ...
        case SID_PITCH_CV_ACTIVE:
            sprintf(tmp, s2_toggle_items[data->pitch_cv_active]);
            menuTFTFlush(4, &_bg);
        ...
        case SID_DIST_ACTIVE:
            sprintf(tmp, s2_toggle_items[data->dist_active]);
            menuTFTFlush(6, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 6); 
            break;
        case SID_DIST:
            sprintf(tmp,"%.2f", s2_dist_q_lut[data->dist_drive]);
...
void s2_menuTFTPrintADSRValues(adsr_data_t* data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
...
void s2_menuTFTPrintFilterValues(filter_data_t* data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    ...
    switch (r)
    {
        case SID_FILTER_ACTIVE:
            ...
            sprintf(tmp, s2_toggle_items[data->is_active]);
            menuTFTFlush(0, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 0);
            break;
        case SID_BASE:
            sprintf(tmp,"%.2f Hz", s2_poti_vals[data->base]);
            menuTFTFlushValue(data->base, 1, filter_conditions, &filter_cond_size, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 1);
            break;
        case SID_WIDTH:
            sprintf(tmp,"%.2f Hz", s2_poti_vals[data->width]);
            menuTFTFlushValue(data->width, 2, filter_conditions, &filter_cond_size, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 2);
            break;
        case SID_Q:
            sprintf(tmp,"%.2f", s2_dist_q_lut[data->q]);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 3);
            break;
...
void s2_menuTFTPrintPlaymodeValues(play_state_data_t *data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    ...
    {
        case SID_MODE:
            sprintf(tmp, s2_playmode_modes[data->mode]);
            menuTFTFlush(0, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 0);
...
void s2_menuTFTPrintMatrixAmount(matrix_ui_row_t* matrix, int r){
...
void s2_menuTFTPrintMatrixDestination(matrix_ui_row_t* matrix, int row){
    ...
    switch(row){
        case PRINT_ALL:
            for(int i = 0; i < 8; i++)
            {
                sprintf(tmp, s2_matrix_parameter_items[matrix[i].dst]); 
                TFT_print(tmp, CENTER, y);
                y += TFT_getfontheight() + 8;
            }
            break;
...
void s2_menuTFTPrintDelayValues(delay_data_t* data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    ...
    switch(r){
        case SID_DELAY_ACTIVE:
            ...
            break;
        case SID_DELAY_MODE:
            sprintf(tmp, s2_delay_mode_items[data->mode]);
            menuTFTFlush(1, &_bg);
            TFT_print(tmp, _width/2, 3 + (TFT_getfontheight() + 3) * 1);
            break;
        case SID_DELAY_TIME:
...
void s2_menuTFTPrintExtInValues(ext_in_data_t* data, int r){
    int y = 0;
    char tmp[16];
    TFT_setclipwin(0,TFT_getfontheight()+9, _width-1, _height);
    _bg = TFT_BLACK;
...
void s2_menuTFTPrintPlaymodeIndicators(play_state_data_t* data, int ind){
    int x1 = 10;
    int x2 = x1 + 300;
    ...
    s2_updateStart(data, loopMarker, prevLoopMarker, &width, &height);
    s2_updateLoopStart(data, loopMarker, prevLoopMarker, &width, &height, ind);
    s2_updateLoopEnd(data, loopMarker, prevLoopMarker, &width, &height);

    s2_updatePrevLoopMarker(loopMarker, prevLoopMarker);

    if(ind == SID_MODE && data->mode == SINGLE) s2_clearLStartMarker(loopMarker, &width, &height);
}

void s2_menuTFTInitPlaymodeIndicators(){
    s2_resetPrevLoopMarker(prevLoopMarker);
}
...
void s2_menuTFTPrintADSRCurve(adsr_data_t* data){
    ...
    //Update Points
    s2_updateAttackPoint(curAdsrPoints, prevAdsrPoints, data, &adsrStartPoint,&width, &height);
    s2_updateDecayPoint(curAdsrPoints, prevAdsrPoints, data, &adsrStartPoint, &width, &height);
    s2_updateSustainPoint(curAdsrPoints, prevAdsrPoints, data, &width, &height);
    s2_updateReleasePoint(curAdsrPoints, prevAdsrPoints, data, &width, &height);

    //Flush indicators
    s2_printIndicatorPoints(prevAdsrPoints, &adsrStartPoint, 2, TFT_BLACK);
    //Reprint
    s2_printIndicatorPoints(curAdsrPoints, &adsrStartPoint, 2, TFT_RED);

    TFT_restoreClipWin();
    s2_updatePrevCurve(curAdsrPoints, prevAdsrPoints);
...
void s2_menuTFTInitAdsrCurve(){
    ...
    s2_resetPrevCurve(prevAdsrPoints);
}

void s2_menuTFTPrintCurrentSettings(param_data_t* data){
    TFT_saveClipWin();
    TFT_resetclipwin();
    _bg = TFT_BLACK;
    _fg = TFT_WHITE;
...
void s2_menuTFTPrintBrowseTagList(list_t *tag_list){
    if(_bbox_list != NULL) list_free(_bbox_list);
    _cur_el = -1;
    _cur_row = 0;
    _bbox_list = list_create();
    ...
    // print all items
    list_each_element(tag_list, s2_printTags);
...
void s2_menuTFTUpdatePlayState(int vid, int state){
    ...
    bool is_playing = !is_rec && !is_armed && s2_isVoicePlaying(vid);
    ...
    s2_audio_get_playpos(vid, &ss, &ls, &le, &fs);
