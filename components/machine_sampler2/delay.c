...
void s2_delay_create(delay_t *delay, float sampleRate, float msMaxLength, delay_cfg_t cfg){
...
void s2_delay_free(delay_t *delay){
    free(delay->bufL);
    free(delay->bufR);
}

void s2_delay_set_params_smooth(delay_t *delay, delay_cfg_t cfg){
    delay->msLength = cfg.msLength;
...
void s2_delay_set_params(delay_t *delay, delay_cfg_t cfg){
    delay->msLength = cfg.msLength;
    //if(delay->msLength > delay->msMaxLength) delay->msLength = delay->msMaxLength;
...
void s2_delay_clear(delay_t *delay){
    memset(delay->bufL, delay->bufLen, sizeof(float));
    memset(delay->bufR, delay->bufLen, sizeof(float));
}

void s2_delay_process(delay_t *delay, const float* bufIn, float* bufOut, uint32_t bufLen){
    float tempL, tempR;
