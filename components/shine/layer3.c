...
#include "types.h"
#include "tables.h"
#include "esp_heap_caps.h"   /* VENDOR PATCH: PSRAM state alloc */
#include "layer3.h"
#include "l3subband.h"
#include "l3mdct.h"
#include "l3loop.h"
...
shine_global_config *shine_initialise(shine_config_t *pub_config)
{
  ...
  /* VENDOR PATCH (ctag-straempler): the ~100 KB encoder state (int2idx LUT
   * dominates) must not eat the ESP32's ~250 KB internal heap - allocate it
   * from PSRAM. Encode hot loops run through the 32 KB PSRAM cache. */
  config = heap_caps_calloc(1, sizeof(shine_global_config),
