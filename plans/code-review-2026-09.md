...
> **Rest of tier 1 + theme D: FIXED 09-16 evening, flashed and bench-run.** `cb19915` theme D (10.1/10.4/10.5: `bpm_analyze` takes the worker run flag, Deck/DoubleDecker analysis on `worker.h`) · `0452d69` 4.1, 6.4, 6.5 · `11810ee` 6.6, 6.7, 6.10 · `4615be4` 3.6, 2.4 · `8c82518` 6.11 (URI swap queued on the httpd task and waited for).
> - **Bench, `tools/bench/review_bench.py` with serial + Scarlett:** 30/30 busy switches incl. Deck auto-analysis and DoubleDecker, every interrupted analysis logs "aborted"; no leak/still-running/panic; a stalled request body released httpd after 25 s (was: forever); 15 switches under URI polling clean; corrupt chunk-size WAV probed without hanging the bus.
> - **Not bench-testable:** 4.1 (needs an unreadable CONFIG.JSN), 6.4 (a wrong WiFi password takes the unit off the LAN: the AP should reappear after ~2 min — hand test).
> - **Found by the bench, not the review:** AUTOSAVE.JSN parses ran out of INTERNAL heap during switches (cJSON used plain malloc, which never reaches PSRAM under CAPS_ALLOC), so autosave skipped most saves. `e9ebea3` made the failure say why, `5b4ea8e` routes cJSON to PSRAM.
> - **Also found by the bench:** menu events queued before `EV_MACHINE_BIND` ran the STOPPED machine's page handlers. Deck's tick started a queued analysis on a stopped Deck, leaking about 22 KB internal, so the next Deck start landed on Stub. Fixed in `2719210` (decks only analyse while live; the menu drops old-page events while a bind is pending).
> - **Final focused run on `2719210`:** 0 fail. 30/30 busy switches, 3 analyses aborted cleanly, **0 autosave errors in 110 saves** (was about 60 per run), Tape 5 takes for 6 punch-outs with NO duplicates (11.5). Watch: internal RAM largest block dips to 10 KB while a BPM analysis runs (its 16 KB DMA chunk + stack).
...
> **ALL FIXED 09-16 evening** (4.2 + 4.4 `336ce36` · 5.2, 2.2, 2.3 `7d6e84f` · 11.4, 11.5 `57556ba` · 3.3, 6.8, 13.3, 13.7, 13.8, 1.4, 1.7 `e327f6f`; 10.1/10.4 above). Bench: autosave persistence across switches, Tape duplicate-take count. 4.2: Settings exit merges its own keys onto a FRESH CONFIG.JSN read; the text-entry page finds ssid/passwd/apikey by POSITION, so they are replaced in place. 5.2: a node with NO matrix is still a partial update (web `/remote/params` contract) and keeps routing; only a stored matrix resets first.
...
> - **2.1 REDONE after the ear test (09-16):** the 95%-of-bound clamp still blew up by ear (a +8..13 dB Chamberlin peak at Nyquist), and Arlo found the SYNTH VOICE filter blowing up below ~35% resonance too (fmax 1.0 with damping > 1.5 is past the bound). His calls: ONE filter everywhere, and one filter at a time. `618d76d` makes the shared svf trapezoidal (same API; svf_coef returns tan and keeps each caller's old top as a frequency cap), and `e594288` hides Filter/Band from the FX slots of Synth/Keys/Tape (`fxrack_t.no_filter`, web `fxnf`). **Arlo, Synth sweep at low resonance: "sounds cleaner now".** Audio cost with a held note: Synth ~315 µs, Keys ~380 µs of 1450.
> - **Ear tests still owed:** a top-of-sweep check on Deck/DoubleDecker/Looper/Slicer filters (no longer bit-identical to older recordings), 2.5 (synced rnd LFO steps every division), 1.10 (a jack clock under 30 BPM at PPQ 1 locks with clk_auto on).
